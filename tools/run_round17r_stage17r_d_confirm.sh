...
TOP_CANDIDATES="${ROUND17R_ROOT}/reports_stage17r_c/round17r_top_candidates.csv"
FT_MANIFEST="${ROUND17R_ROOT}/manifests/stage17r_d_finetune_dispatch_manifest.csv"
...
echo "========== ROUND17R STAGE 17R-D CONFIRM START $(date -u +%Y-%m-%dT%H:%M:%SZ) =========="
echo "FINETUNE_PARALLEL=${FINETUNE_PARALLEL}"

if [[ ! -f "${TOP_CANDIDATES}" ]]; then
  echo "ERROR: missing ${TOP_CANDIDATES} (run 17R-C first)"
  exit 1
fi

r17_notify --event stage-start --stage 17R-D --manifest "${FT_MANIFEST}"
...
python3 tools/optimization_runner.py finetune \
  ...
  --run-dir "${ROUND17R_ROOT}/stage17r_d" \
...
python3 tools/analyze_round17r_18class.py \
  ...
  --settings "${SETTINGS}" \
  --aggregate "${ROUND17R_ROOT}/stage17r_d/aggregate/aggregate_scores.csv" \
  --stage 17r_d \
  --outdir "${ROUND17R_ROOT}/reports_stage17r_d"

r17_notify --event stage-done --stage 17R-D --manifest "${FT_MANIFEST}"
trap - EXIT
echo "========== ROUND17R STAGE 17R-D CONFIRM DONE $(date -u +%Y-%m-%dT%H:%M:%SZ) =========="
