...
LOG="${ROUND11_ROOT}/logs/pipeline_resume.log"

mkdir -p "${ROUND11_ROOT}/logs"

{
  echo "========== ROUND11 RESUME $(date -u +%Y-%m-%dT%H:%M:%SZ) =========="

  echo "[Round11] Run pretrain (pending only)"
  ...
  python3 tools/optimization_runner.py aggregate --run-dir "${ROUND11_ROOT}"
  python3 tools/optimization_runner.py report --run-dir "${ROUND11_ROOT}"
