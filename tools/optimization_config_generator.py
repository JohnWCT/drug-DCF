...
# Backward-compatible default columns (Round 3 manifests); dynamic sweep keys are appended.
MANIFEST_COLUMNS = [
    "job_id",
    "config_path",
    "lambda_proto",
...
def _manifest_row_from_combo(job_id: str, config_path: str, combo: dict) -> dict:
    combo_norm = dict(combo)
    if "proto_min_samples_per_class" not in combo_norm:
        combo_norm["proto_min_samples_per_class"] = combo_norm.get("min_proto_samples_per_class", 1)
    row = {
        "job_id": job_id,
        "config_path": config_path,
    }
    for key in MANIFEST_COLUMNS:
        if key in ("job_id", "config_path"):
            continue
        if key in combo_norm:
            row[key] = combo_norm[key]
        elif key in MANIFEST_STATUS_COLUMNS:
            row[key] = ""
    for key, value in combo_norm.items():
        if key not in row:
            row[key] = value
    if "status" not in row or row["status"] == "":
        row["status"] = "pending"
    return row


def _order_manifest_columns(df: pd.DataFrame) -> pd.DataFrame:
    ordered = [c for c in MANIFEST_COLUMNS if c in df.columns]
...
def generate_configs(
    ...
    for idx, combo in enumerate(expand_sweep_combinations(spec["sweep"])):
        ...
        rows.append(_manifest_row_from_combo(job_id, rel_config_path, combo))

    manifest_df = pd.DataFrame(rows)
    manifest_df = _order_manifest_columns(manifest_df)
    if existing_manifest is not None and not force:
        ...
        for _, new_row in manifest_df.iterrows():
            ...
            if not mask.any():
                ...
                preserve_cols = MANIFEST_STATUS_COLUMNS
