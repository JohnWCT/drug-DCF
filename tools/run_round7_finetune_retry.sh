#!/usr/bin/env bash
# Retry failed Round 7 finetune jobs → aggregate → report.
set -euo pipefail
cd "$(dirname "$0")/.."
...
LOG="${RUN_DIR}/logs/round7_finetune_retry.log"
FINETUNE_BATCH_SIZE="${FINETUNE_BATCH_SIZE}"
FINETUNE_MINI_BATCH="${FINETUNE_MINI_BATCH}"
# Finetune: 42 caused CUBLAS_STATUS_NOT_INITIALIZED on 120-job Round 7 grid; use 26.
FINETUNE_MAX_PARALLEL="${FINETUNE_RETRY_PARALLEL:-26}"

mkdir -p "${RUN_DIR}/logs"
exec > >(tee -a "${LOG}") 2>&1

echo "========== ROUND7 FINETUNE RETRY $(date -u +%Y-%m-%dT%H:%M:%SZ) =========="
...
python3 tools/update_running_report.py --run-dir "${RUN_DIR}" \
  --note "Round7 finetune retry complete (max_parallel=${FINETUNE_MAX_PARALLEL})." || true
