...
def smile_to_graph(smile):
    ...
    # Desalt: if SMILES contains multiple fragments (salt forms),
    # keep only the largest fragment by atom count.
    # e.g. cisplatin "N.N.[Cl-].[Cl-].[Pt+2]" -> keep largest fragment
    if mol is not None and '.' in smile:
...
def append_csv_log(file_name, data_dict):
    """
    Append data to a CSV file with headers.
    If the file doesn't exist, it will create it with headers.
    
    Args:
        file_name (str): Path to the CSV file
        data_dict (dict): Dictionary containing data to append
    """
    import os
    import csv
    import pandas as pd
    ...
    for key, value in data_dict.items():
        if hasattr(value, 'item'):  # Check if it's a tensor
            try:
                clean_dict[key] = float(value.item())  # Extract scalar value from tensor
            except:
                clean_dict[key] = float(value.detach().cpu().numpy())  # Fallback method
        else:
    ...
    if file_exists:
        ...
        # Make sure all columns in data_dict exist in headers
        # If not, need to create a new file with updated headers
        if not all(key in headers for key in clean_dict.keys()):
            # Read existing data
        ...
        with open(file_name, 'w', newline='') as f:
            writer = csv.DictWriter(f, fieldnames=clean_dict.keys())
            writer.writeheader()
            writer.writerow(clean_dict)
...
def calculate_fid(real_features, fake_features):
    ...
    Args:
        real_features (torch.Tensor): Features from real data
        fake_features (torch.Tensor): Features from generated data
...
def calculate_mmd(real_features, fake_features, kernel='rbf'):
    """
    ...
    try:
        ...
        if real_features.shape[1] == 0 or fake_features.shape[1] == 0:
            return float('inf')
        ...
        if kernel == 'rbf':
            # RBF kernel
            gamma = 1.0 / real_features.shape[1]  # 1/feature_dim
            K_xx = np.exp(-gamma * np.sum((real_features[:, np.newaxis] - real_features[np.newaxis, :]) ** 2, axis=2))
            K_yy = np.exp(-gamma * np.sum((fake_features[:, np.newaxis] - fake_features[np.newaxis, :]) ** 2, axis=2))
            K_xy = np.exp(-gamma * np.sum((real_features[:, np.newaxis] - fake_features[np.newaxis, :]) ** 2, axis=2))
        else:
            # Linear kernel
            K_xx = np.dot(real_features, real_features.T)
            K_yy = np.dot(fake_features, fake_features.T)
            K_xy = np.dot(real_features, fake_features.T)
        ...
        # Handle numerical errors
        if np.isnan(mmd) or np.isinf(mmd):
            return float('inf')
...
def calculate_wasserstein(real_features, fake_features):
    """
    Calculate Wasserstein distance between real and fake features
    ...
    try:
        ...
        try:
            sqrt_term = linalg.sqrtm(sigma_real.dot(sigma_fake))
            if np.iscomplexobj(sqrt_term):
                sqrt_term = sqrt_term.real
            if np.isnan(sqrt_term).any() or np.isinf(sqrt_term).any():
                return float('inf')
            cov_diff = np.trace(sigma_real + sigma_fake - 2 * sqrt_term)
        except:
            return float('inf')
        
        wasserstein = mean_diff + cov_diff
        
        # Handle numerical errors
        if np.isnan(wasserstein) or np.isinf(wasserstein):
            return float('inf')
