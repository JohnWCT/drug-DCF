import torch
from biocda.models.model_factory import build_cross_attention_model
...
def test_checkpoint_roundtrip(xa_config, tmp_path):
    ...
    model2 = build_cross_attention_model(xa_config)
...
def test_architecture_version_saved(xa_config, tmp_path):
    ckpt = tmp_path / "model.pt"
    save_biocda_checkpoint(ckpt, model=build_cross_attention_model(xa_config), config=xa_config, epoch=0, architecture_version="biocda-xa-v1")
...
def test_encoder_hashes_saved(xa_config, tmp_path):
    ckpt = tmp_path / "model.pt"
    save_biocda_checkpoint(ckpt, model=build_cross_attention_model(xa_config), config=xa_config, epoch=0, omics_encoder_sha="o", drug_encoder_sha="d", context_artifact_sha="c")
    blob = torch.load(ckpt, map_location="cpu")
    assert blob["omics_encoder_sha"] == "o" and blob["drug_encoder_sha"] == "d"

def test_config_saved_in_checkpoint(xa_config, tmp_path):
    ckpt = tmp_path / "model.pt"
    save_biocda_checkpoint(ckpt, model=build_cross_attention_model(xa_config), config=xa_config, epoch=0)
    assert torch.load(ckpt, map_location="cpu")["config"]["model"]["type"] == "cross_attention"
