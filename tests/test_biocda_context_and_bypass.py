...
def test_context_swap_can_change_attention(xa_model, sample_batch):
    omics, context, batch = sample_batch
    a = xa_model(omics, context, batch, output_mode="full")
    shuffled = context.flip(0)
    b = xa_model(omics, shuffled, batch, output_mode="full")
    assert not torch.allclose(a.sample_representation, b.sample_representation)
